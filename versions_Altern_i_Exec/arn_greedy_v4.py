...
def is_better22(k, pos_pen, next_k, next_pen, prop_next, pens, quant, used) -> bool:
    if pos_pen < next_pen:
        return True
    if pos_pen > next_pen:
        return False
...
def greedy(
    ...
    for i in range(C):
        ...
        for k in range(K):
            if used[k] < quant[k]:
                val[k] = 0.
                pens[k] = -add_pen(M, i+1, ne, mill_clas, act_sol + [k], ce)
                for m in range(M):
                    if mill_clas[k][m]:
                        val[k] += rem[m]
        tries = argmax(pens)
        ...
        act_pen += add_pen(M, len(act_sol), ne, mill_clas, act_sol, ce)
    for m in range(M):
        count = 0
