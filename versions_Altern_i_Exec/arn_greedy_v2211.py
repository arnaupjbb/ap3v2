...
def is_better22(k, pos_pen, next_k, next_pen, prop_next, pens, quant, used, addprox, next_addprox) -> bool:
    if pos_pen < next_pen:
        return True
    if pos_pen > next_pen:
        return False
    if addprox < next_addprox:
        return True
    if addprox > next_addprox:
        return False
    ...
    for i in range(C):
        ...
        for k in range(K):
            if used[k] < quant[k] :
                ...
                if is_better22(k, pos_pen, next_k, next_pen, prop_next, pens, quant, used, addprox, next_pen_add):
                    next_pen = pos_pen
