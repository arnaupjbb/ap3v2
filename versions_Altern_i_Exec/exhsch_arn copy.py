...
def add_pen(M, L, ne, mill_clas, act_sol, ce, cre:list[int]) -> tuple[int, int]:
...
def exh_sch(
        ...
        start, arch, addprox:int, cre: list[int]
        ):
    ...
    for k in range(K):
        if used[k] < quant[k]:
            ...
            new_p, addprox = add_pen(M, L + 1, ne, mill_clas, act_sol, ce, cre)
            best_pen = exh_sch(
                C, M, K, ce, ne, quant, mill_clas,
                act_pen + new_p, act_sol, used,
                best_pen, start, arch, addprox, cre
                )
