...
def exh_sch(
        ...
        start, arch, addprox:int, rem: list[int] cre: list[int]
        ):
    ...
    to found it"""
    if act_pen + addprox +  >= best_pen or (0 < len(act_sol) < C  and rem[act_sol[0]] == 0):
        return best_pen
    L: int = len(act_sol)
...
def main():
    ...
    for i in range(K-2, -1, -1):
        rem[i] = rem[i+1] + quant[i]

    _ = exh_sch(
        C, M, K, ce, ne, quant, mill_clas,
        0, [],[0]*K, C*C*M, start, arch, 0, rem
        )
