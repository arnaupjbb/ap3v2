        ...
        for k in range(K):
            if used[k] < quant[k] :
                ...
                for m in range(M):
                    if mill_clas[k][m]:
                        val[k] += rem[m]
                if undecadax != 0 and random.randint(1, undecadax) == 1:
                    next_pen = pos_pen
...
def metaheur(C, M, K, ce, ne, quant, mill_clas, pens, arch, start):
    best_pen, best_sol = rangreedy(C, M, K, ce, ne, quant, mill_clas, pens, 0)
    ...
    while True:
        ...
        if iter > 10000:
            T = 10.
            new_pen, new_sol = rangreedy(C, M, K, ce, ne, quant, mill_clas, pens, K//2+1)
            iter = 0
