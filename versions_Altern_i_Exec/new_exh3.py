...
def main():
    ...
    def find_best_sol_rec(dislike, idx, lasts) -> int :
        ...
        if idx == C :
            ...
            for k in range(K) :
                if used[k] < quant[k]:
                    ...
                    best_cost = find_best_sol_rec(dislike + pen, idx + 1, lasts) #falta fer dislike + penalty
                    for m in range(M):
                        if mill_clas[k][m]: lasts[m] -= 1
                        if idx >= ne[m] and mill_clas[sol[idx - ne[m]]][m]: lasts[m] += 1
                    used[k] -= 1
    ...
    cost = find_best_sol_rec(0, idx, lasts)
