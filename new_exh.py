...
def find_best_sol_rec(ce, ne, quant, mill_clas, used, sol, dislike, best_cost, idx, lasts) -> int :
    C, M, K = len(sol), len(mill_clas[0]), len(used)
    if dislike  >= best_cost : return best_cost
    ...
    if idx == C :
        ...
        for k in range(K) :
            if used[k] < quant[k]:
                ...
                best_cost = find_best_sol_rec(ce, ne, quant, mill_clas, used, sol, dislike + pen,best_cost, idx + 1, lasts) #falta fer dislike + penalty
                for m in range(M):
                    if mill_clas[k][m]: lasts[m] -= 1
                    if idx >= ne[m] and mill_clas[sol[idx - ne[m]]][m]: lasts[m] += 1
                used[k] -= 1
...
def main():
    ...
    best_cost = C*C*M
    lasts = [0]*M
    cost = find_best_sol_rec(ce, ne, quant, mill_clas, used, sol, dislike, best_cost, idx, lasts)
