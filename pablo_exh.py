...
def min_pen_rec(sol: list[int], classes_restants: list[int], millores: list[list[int]], cost_actual: int, idx: int, ce: list[int], ne: list[int], min_cost: list[int], inici: int):
    C = len(sol)
    M = len(millores[0])
    K = len(millores)
    if idx == C:
        ...
        for i in range(K):
            if classes_restants[i] != 0:
                nou_cotxe = i
                print(classes_restants, sol, idx)
                classes_restants[i] -= 1
                sol[idx] = nou_cotxe
                nc = nou_cost(sol, millores, idx, ce, ne)
                cost, new_sol = min_pen_rec(sol, classes_restants, millores, cost_actual+nc, idx+1, ce, ne, min_cost)
                if cost < best_cost:
                    best_cost = cost
                    best_sol = new_sol
                sol[idx] = -1
...
def min_pen(C: int, cotxes_classe: list[int], millores: list[list[int]], ce: list[int], ne: list[int]):
    sol = [-1]*C
...
def main():
    ...
    cost, sol= min_pen(C, cotxes_classe, millores, ce, ne)
    print(cost, sol)
